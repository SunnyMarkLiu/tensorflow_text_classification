...
with tf.Graph().as_default(), tf.device('/gpu:1'):
    ...
    with session.as_default():
        bilstm = BiLSTM(
            ...
            lstm_drop_out=FLAGS.lstm_drop_out
        )
        # Define global training procedure
        global_step = tf.Variable(0, name="global_step", trainable=False)

        optimizer = tf.train.AdamOptimizer(learning_rate=bilstm.learning_rate)
        grads_and_vars = optimizer.compute_gradients(bilstm.loss)  # Compute gradients of `loss` for the variables in `var_list`.

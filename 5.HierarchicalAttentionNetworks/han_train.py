...
tf.flags.DEFINE_integer('num_sentences', 10, 'The number of sentences, document splited (default: 10)')
...
tf.flags.DEFINE_integer('word_encoder_bigru_num_units', 200, 'The hidden layer size about bi-gru cell (default: 200)')
tf.flags.DEFINE_integer('word_attention_size', 100, 'The word level attention size (default: 100)')
tf.flags.DEFINE_integer('sent_attention_size', 100, 'The sentence level attention size (default: 100)')
tf.flags.DEFINE_float('dropout_keep_prob', 0.6, "Dropout keep probability about attention layer(default: 0.6)")
...
tf.flags.DEFINE_float("l2_reg_lambda", 1e-2, "L2 regulaization rate (default: 1e-2)")
...
with tf.Graph().as_default(), tf.device('/gpu:1'):
    ...
    with session.as_default():
        han_model = HierarchicalAttentionNetworks(
            label_size=2,
            num_sentences=FLAGS.num_sentences,
            sequence_length=max_document_length,
            vocabulary_size=vocabulary_size,
            embedding_dim=FLAGS.embedding_dim,
            word_attention_size=FLAGS.word_attention_size,
            sent_attention_size=FLAGS.sent_attention_size,
            word_encoder_bigru_num_units=FLAGS.word_encoder_bigru_num_units,
            embedding_trainable=FLAGS.embedding_trainable
        )
        # Define global training procedure
        global_step = tf.Variable(0, name="global_step", trainable=False)

        optimizer = tf.train.AdamOptimizer(learning_rate=han_model.learning_rate)
        grads_and_vars = optimizer.compute_gradients(han_model.loss)  # Compute gradients of `loss` for the variables in `var_list`.
        ...
        learning_rate_summary = tf.summary.scalar("learning_rate", han_model.learning_rate)
        loss_summary = tf.summary.scalar("loss", han_model.loss)
        accuracy_summary = tf.summary.scalar("accuracy", han_model.accuracy)
        ...
        session.run(tf.assign(ref=han_model.embedding_matrix, value=word_embeddings))

        def train_step(x_batch, y_batch, learning_rate_, dropout_keep_prob):
            """ training step """
            feed_dict = {
                han_model.inputs: x_batch,
                han_model.labels: y_batch,
                han_model.learning_rate: learning_rate_,
            ...
            _, step, summaries, loss_, accuracy_ = session.run([train_op, global_step, train_summary_op,
                                                                han_model.loss, han_model.accuracy],
                                                                feed_dict=feed_dict)
        ...
        for epoch in range(FLAGS.epochs):
            ...
            for i in range(x_train.shape[0] // FLAGS.batch_size):
                ...
                if current_step % FLAGS.evaluate_every_steps == 0:
                    ...
                    print(
                        "---> valid {}: step: {:d}, loss {:g}, acc {:g}".format(time_str, current_step, np.mean(losses),
                                                                                np.mean(accuracies)))
        ...
        def test_step(x_batch, y_batch):
            ...
            accuracy_ = session.run(han_model.accuracy, feed_dict=feed_dict)
            return accuracy_
